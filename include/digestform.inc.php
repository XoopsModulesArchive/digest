...
$sform = new XoopsThemeForm(_AM_DIGEST_SITE, "digestform", xoops_getenv('PHP_SELF'));
$sform->setExtra('enctype="multipart/form-data"');

$sform->addElement(new XoopsFormText(_AM_DIGEST_TITLE, 'title', 50, 80, $title), true);
$sform->addElement(new XoopsFormText(_AM_DIGEST_URL, 'url', 80, 255, $url), true);
$sform->addElement(new XoopsFormText(_AM_DIGEST_DESCRIPTION, 'description', 80, 255, $description), false);

ob_start();
$mytree = new XoopsTree($xoopsDB->prefix("digest_categories"), "category_id", "0");
$mytree->makeMySelBox("title", "category_id", $category_id);
$sform->addElement(new XoopsFormLabel(_AM_DIGEST_CATEGORY, ob_get_contents()));
ob_end_clean();
...
if (is_object($xoopsUser) && $xoopsUser->isAdmin()) {
	$sform->addElement(new XoopsFormRadioYN(_AM_DIGEST_ONLINE, 'online', $online,'' . _YES . '', ' ' . _NO . '' ), false);
	$sform->addElement(new XoopsFormText(_AM_DIGEST_ORDER, 'digest_order', 20, 20, $digest_order), false);
}else{
	$sform->addElement(new XoopsFormHidden('online', $online));
	$sform->addElement(new XoopsFormHidden('digest_order', $digest_order));
}
$sform->addElement(new XoopsFormText(_AM_DIGEST_OFFSET, 'offset', 20, 20, $offset), false);
$sform->addElement(new XoopsFormText(_AM_DIGEST_MAXITEMS, 'maxitems', 20, 20, $maxitems), false);
$sform->addElement(new XoopsFormText(_AM_DIGEST_MINLENGTH, 'minlength', 20, 20, $minlength), false);
$sform->addElement(new XoopsFormText(_AM_DIGEST_CHARSET, 'charset', 20, 20, $charset), false);
$sform->addElement(new XoopsFormText(_AM_DIGEST_CHARSET_INTER, 'charset_inter', 20, 20, $charset_inter), false);
$sform->addElement(new XoopsFormText(_AM_DIGEST_UPDATETIME, 'updatetime', 20, 20, $updatetime), true);
$sform->addElement(new XoopsFormText(_AM_DIGEST_REGEXP, 'reg_exp', 80, 255, $reg_exp), false);
$sform->addElement(new XoopsFormText(_AM_DIGEST_CRITERIA, 'criteria', 80, 255, $criteria), false);
...
$button_tray->addElement(new XoopsFormButton('', 'fetch', _AM_DIGEST_FETCH, 'submit'));
