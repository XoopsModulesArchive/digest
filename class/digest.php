...
// ------------------------------------------------------------------------- //
if(defined("DIGEST_DIGEST")) return;
define("DIGEST_DIGEST", 1);
...
class Digest extends XoopsObject
{
	...
	function Digest()
	{
		...
		$this->table = $this->db->prefix("digest_digests");
		...
		$this->initVar('offset', XOBJ_DTYPE_INT);
		$this->initVar('maxitems', XOBJ_DTYPE_INT);
		$this->initVar('minlength', XOBJ_DTYPE_INT);
		$this->initVar('updatetime', XOBJ_DTYPE_INT);
		$this->initVar('lastupdate', XOBJ_DTYPE_INT);
		$this->initVar('reg_exp', XOBJ_DTYPE_TXTBOX);
		$this->initVar('criteria', XOBJ_DTYPE_TXTBOX);
	...
	{
		$item_handler =& xoops_getmodulehandler('item', 'digest');
		$item =& $item_handler->get($this->getVar('url'));
		if(!is_object($item)) {
			//echo "<br />_digestItems::item: create error";
			return false;
		}
		if($this->getVar('charset')) $item->setVar('charset', $this->getVar('charset'));
		$item->setVar('offset', $this->getVar('offset'));
		$item->setVar('reg_exp', $this->getVar('reg_exp'));
		$item->setVar('criteria', $this->getVar('criteria'));
		$item->setVar('maxitems', $this->getVar('maxitems'));
		$item->setVar('minlength', $this->getVar('minlength'));
		...
		if(!$this->getVar('charset')) $this->setVar('charset', $item->getVar('charset'));
		$item_handler->destroy($item);
...
class DigestDigestHandler extends XoopsObjectHandler
{
    ...
    {
    	$sql = 'SELECT * FROM '.$this->db->prefix('digest_digests');
	...
	{
		$sql = "SELECT COUNT(*) FROM ".$this->db->prefix("digest_digests");
		$criteria_id = (intval($id))?" category_id=".intval($id):"";
		$criteria_online = ($isOnline)?" online =1":"";
		if ( !empty($criteria_id) || !empty($criteria_online) ) {
			$sql .= " WHERE ".$criteria_id;
       ...
       if ( $digest->isNew() )
        {
            ...
            $sql = "INSERT INTO ".$digest->table."
            			( digest_id,  category_id,  digest_order,  online,  title,  description,  url,  image,  offset,  maxitems,  minlength,  charset,  charset_inter,  updatetime,  reg_exp,  criteria)
					VALUES
                    	($digest_id, $category_id, $digest_order, $online, $title, $description, $url, $image, $offset, $maxitems, $minlength, $charset, $charset_inter, $updatetime, $reg_exp, $criteria)";
          	if ( !$result = $this->db->queryF($sql) ) {
			...
			$sql = "UPDATE ".$digest->table." SET digest_order = $digest_order, online = $online, title = $title, url = $url,	description = $description, image = $image, offset = $offset, maxitems = $maxitems, minlength = $minlength, charset = $charset, charset_inter = $charset_inter, updatetime = $updatetime, lastupdate = 0, reg_exp = $reg_exp, criteria = $criteria, category_id = $category_id, items= '' WHERE digest_id = ".$digest->getVar('digest_id');
            if ( !$result = $this->db->queryF($sql) ) {
