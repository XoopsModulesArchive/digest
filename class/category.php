...
class Category extends XoopsObject
{
    function Category() 
    {
        ...
        $this->table = $this -> db -> prefix( "digest_categories" );
...
class DigestCategoryHandler extends XoopsObjectHandler
{
    ...
    function &get($id) 
    {
        ...
        $sql = 'SELECT * FROM '.$this->db->prefix('digest_categories').' WHERE category_id='.$id;
        if($result = $this->db->query($sql) && $array = $this->db->fetchArray($result)){
    ...
    function &getAll($order = "category_order") 
    {
    	...
    	$sql = 'SELECT * FROM '.$this->db->prefix('digest_categories')." ORDER BY $order, category_id";
    ...
    function &getItemsByCategory($category=0)
    {
    	...
    	$sql = 'SELECT * FROM '.$this->db->prefix('digest_digests').$id;
        $result = $this->db->query($sql);
        $ret = array();
        while ($myrow = $this->db->fetchArray($result)) {
            $category =& $this->create(false);
    ...
    {
	    $category = null;
        $sql = "SELECT t.* FROM ".$this->db->prefix('digest_categories')." t, ".$this->db->prefix('digest_digests')." d WHERE t.category_id = d.category_id AND d.digest_id = ".intval($id);
    ...
    function getDigestCount($category=null)
    {
	    $cat_id = is_object($category)?$category->getVar('category_id'):intval($category);
	    $id= empty($cat_id)?'':" WHERE category_id=".$cat_id;
        $result = $this->db->query("SELECT COUNT(storyid) FROM ".$db->prefix('digest_digests').$id);
    ...
    {
        $result = $this->db->query("SELECT COUNT(category_id) FROM ".$this->db->prefix('digest_categories'));
        list($count) = $this->db->fetchRow($result);
		return $count;
    }
